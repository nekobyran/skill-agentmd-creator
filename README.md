...
`design-create`、`design-update` 与 `design-source-update` 在 `normative` 模式下以规范治理约束输出结构与质量，但规则正文采用自由语义，不要求或偏好任何固定句式：

- 保留用户要求中的真实意图、范围、强度、触发边界、排除项、顺序和验证义务。
- 只有规则本身确实依赖条件时才显式写条件；只有流程或验证语义真实存在时才写步骤、验证或证据要求。
- 不为了格式统一而编造条件、步骤、验证义务或其他结构，语义忠实度优先于句式一致性。
- `normative` 仍负责分区结构、固定编号、去重、规则膨胀审查、强约束保留和 bundle 完整性，而不是把规则文本变成某种 DSL 模板。

因此 CLI 可以直接从自然语言需求生成规范 Skill，同时让每条规则使用最适合当前场景的自由表达。原始 `create` / `update` 是无损 bundle 同步入口，不执行上述 AI 语义规范化。
