...
pub fn design_skill_at(
    ...
    let user_content = if current_source.is_empty() {
        format!("Mode: {mode}\nUser request:\n{prompt}")
    } else {
        format!(
    ...
    let instructions = concat!(
        ...
        "For create mode, design a complete production-ready SKILL.md. For modify mode, return the complete revised SKILL.md and preserve all unrelated or unknown sections, nested lists, code fences, and references from the supplied source. ",
        "The Markdown must begin with valid YAML frontmatter containing non-empty name and description fields. Keep operational detail, triggers, scope, workflow steps, properties, validation, failure handling, and evidence requirements when relevant. ",
        "Return exactly one JSON object with two string fields: assistantMessage (a concise Chinese explanation of the proposal) and markdown (the entire proposed SKILL.md). Do not wrap the JSON in Markdown fences and do not add text outside the JSON."
    );
    ...
    for rule in rules {
        ...
        if rule.editor_type == "rule" {
            for route in &rule.rule_trigger_routes {
                ...
                rendered.push(format!("若 {} 则 {trigger_content}", limits.join(joiner)));
            }
            continue;
        }
...
fn write_entry_manifest(root: &Path) -> Result<PathBuf, String> {
  ...
  "args": {
    ...
    fs::write(&entry_path, json.as_bytes()).map_err(|error| error.to_string())?;
    Ok(entry_path)
}
...
mod tests {
    ...
    fn renders_rule_trigger_routes_with_all_and_any_limits() {
        ...
        assert_eq!(rendered, vec!["若 2 且 3 则 4", "若 5 或 6 则 4"]);
    ...
    fn separates_structured_rule_cards_with_blank_lines() {
        ...
        assert!(markdown.contains("- 若 limit-1 则 trigger-1\n\n- 若 limit-2 则 trigger-2"));
    }
